...
public enum PopupCustomOption {
    case layout(GTPopController.PopupLayout)
    case animation(GTPopController.PopupAnimation)
    case backgroundStyle(GTPopController.PopupBackgroundStyle)
...
// MARK: - Protocols
/** GT:
    Every ViewController which is added on the GTPopController must need to be conformed this protocol.
 */
public protocol GTPopControllerDelegate {
    
    /** sizeForPopup(GTPopController: size: showingKeyboard:):
        return view's size
     */
    func sizeForPopup(_ GTPopController: GTPopController, size: CGSize, showingKeyboard: Bool) -> CGSize
}

open class GTPopController: UIViewController {
    ...
    fileprivate var closedHandler: ((GTPopController) -> Void)?
    fileprivate var showedHandler: ((GTPopController) -> Void)?
...
// MARK: - Publics
public extension GTPopController {
    
    // MARK: Classes
    public class func create(_ parentViewController: UIViewController) -> GTPopController {
        let controller = GTPopController()
    ...
    public func customize(_ options: [PopupCustomOption]) -> GTPopController {
        customOptions(options)
        return self
    }

    @discardableResult
    public func show(_ childViewController: UIViewController) -> GTPopController {
    ...
    public func didShowHandler(_ handler: @escaping (GTPopController) -> Void) -> GTPopController {
        self.showedHandler = handler
        return self
    }
    
    public func didCloseHandler(_ handler: @escaping (GTPopController) -> Void) -> GTPopController {
...
// MARK: Privates
private extension GTPopController {
    ...
    func registerNotification() {
        NotificationCenter.default.addObserver(self, selector: #selector(GTPopController.GTPopControllerWillShowKeyboard(_:)), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(GTPopController.GTPopControllerWillHideKeyboard(_:)), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(GTPopController.GTPopControllerDidHideKeyboard(_:)), name: NSNotification.Name.UIKeyboardDidHide, object: nil)
    }
    ...
    func registerTapGesture() {
        let gestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(GTPopController.didTapGesture(_:)))
    ...
    func updateLayouts() {
        guard let child = self.childViewControllers.last as? GTPopControllerDelegate else { return }
    ...
    @objc func GTPopControllerWillShowKeyboard(_ notification: Notification) {
    ...
    @objc func GTPopControllerWillHideKeyboard(_ notification: Notification) {
        back()
    }
    
    @objc func GTPopControllerDidHideKeyboard(_ notification: Notification) {
    ...
    func show(_ layout: PopupLayout, animation: PopupAnimation, completion: @escaping PopupAnimateCompletion) {
        guard let childViewController = childViewControllers.last as? GTPopControllerDelegate else {
            return
        }
    ...
    func hide(_ animation: PopupAnimation, completion: @escaping PopupAnimateCompletion) {
        guard let child = childViewControllers.last as? GTPopControllerDelegate else {
            return
        }
...
extension GTPopController: UIScrollViewDelegate {
...
extension GTPopController: UIGestureRecognizerDelegate {
...
//    func GTPopController() -> GTPopController? {
//        var parent = parent
//        while !(parent is GTPopController || parent == nil) {
//            parent = parent!.parent
//        }
//        return parent as? GTPopController
//    }
//}
